...
let general = ["Reconocimiento de que cepillarse los dientes los limpia; previene el deterioro, enfermedad, caries y otros problemas dentales; o mejora la apariencia",
              ...
              "Para conservar una amistad al admitir tu culpa e intentar que la otra persona se sienta mejor al tener consciencia de tu pena y de que buscas perdón",
...
function Comprension(props) {
  ...
  function getResult() {
    ...
    if(!props.resWechsler.hasOwnProperty('CM')){
      props.setResWechsler('CM',total)
    }else{
      if(props.resWechsler['CM'] !== total){
        props.setResWechsler('CM',total)
      }
    }
        ...
        <div > 
          ...
          <div id="generalC">
            ...
            {general[numberItem].split("\n\n").map((i,key) => {
              return <div key={key}>
                  &nbsp;
                  <Typography variant="body2" component="p"> {key+1}. {i} </Typography> 
                </div>;
            })}
          </div>
          <br/>
